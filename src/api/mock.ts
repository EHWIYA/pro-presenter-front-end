...
const MOCK_LIBRARY_SONGS: SongDetail[] = [
  ...
  {
    songId: '성가곡/나의 기도하는 것을',
    title: '나의 기도하는 것을',
    artist: null,
    category: 'hymn',
    tags: [],
    libraryCategory: '성가곡',
    presentationFilename: '나의 기도하는 것을.pro',
    sections: [
      {
        type: 'verse',
        label: '1절',
  ...
  },
  {
    ...
    tags: [],
    libraryCategory: '찬양',
    presentationFilename: '은혜 아니면.pro',
    sections: [
  ...
  },
  {
    ...
    tags: [],
    libraryCategory: '찬송가',
    presentationFilename: '413.내 평생.pro',
    sections: [
      { type: 'verse', label: '1절', lines: ['내 평생 가는 길', '주와 함께'] },
      { type: 'verse', label: '2절', lines: ['주의 손 잡고', '걸어가리'] },
    ],
...
export async function mockFetchSongs(
  ...
  const filtered = MOCK_LIBRARY_SONGS.filter((s) => {
    ...
    if (libraryCategory && s.libraryCategory !== libraryCategory) return false;
  ...
  const items = filtered.slice(offset, offset + limit).map((s) => ({
    ...
    category: s.category,
    tags: s.tags,
    libraryCategory: s.libraryCategory,
    presentationFilename: s.presentationFilename,
...
export async function mockFetchSong(
  songId: string,
  venueId?: string,
): Promise<SongDetail> {
  await delay(200);
  ...
  const includeSections = Boolean(venueId);
  return {
    ...found,
    sections: includeSections
      ? found.sections.map((s) => ({ ...s, lines: [...s.lines] }))
      : [],
    sectionsHint: includeSections
      ? undefined
      : 'venueId를 지정하면 에이전트가 .pro에서 구간을 불러옵니다.',
  };
}

export async function mockFetchLibrarySongSections(
  _venueId: string,
  songId: string,
): Promise<import('./types').LibrarySongSectionsResponse> {
  await delay(280);
  const found = MOCK_LIBRARY_SONGS.find((s) => s.songId === songId);
  if (!found) {
    throw new Error('곡을 찾을 수 없습니다.');
  ...
  return {
    songId: found.songId,
    libraryCategory: found.libraryCategory,
    sections: found.sections.map((s) => ({ ...s, lines: [...s.lines] })),
  };
}
...
export async function mockGetSongJob(jobId: string): Promise<SongJobResponse> {
  ...
  return {
    ...
    libraryAction: 'skipped',
    libraryReason: 'data-repo',
  };
}
